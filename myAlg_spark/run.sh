...
if [ $1 == "run" ]; then
    ...
    ${RCMD} --class org.apache.spark.examples.mllib.FPGrowthExample --master ${ADDRESS}  ${CONF} /usr/local/spark/lib/spark-examples-1.3.1-hadoop2.6.0.jar --minSupport ${MINSUP} --numPartition 1 ${DB2}
fi
